...
const GEOJSON_URL = "/tis_poligonos.geojson";
...
export default function MapViewTI() {
  ...
  useEffect(() => {
    ...
    const adicionarCamadas = () => {
      ...
      try {
        map.addSource("tis", { type: "geojson", data: dados as any });
        ...
        map.on("click", "tis-fill", (e) => {
          const feature = e.features?.[0];
          if (!feature) return;
          const p = feature.properties || {};
          const nome = escapar(p.nome);
          const slug = p.slug ? escapar(p.slug) : null;
          const situacao = escapar(p.situacao);
          const modalidade = escapar(p.modalidade);
          ...
          detalhe.textContent = [situacao, modalidade, uf].filter(Boolean).join(" · ");
          root.appendChild(detalhe);

          if (slug) {
            const link = document.createElement("a");
            link.href = `/terras-indigenas/${slug}`;
            link.className = "mt-2 inline-block text-xs font-medium text-primary-600 underline";
            link.textContent = "Abrir ficha da Terra Indígena →";
        ...
        map.on("mouseenter", "tis-fill", () => {
          map.getCanvas().style.cursor = "pointer";
        });
        map.on("mouseleave", "tis-fill", () => {
  ...
  return (
    <div className="relative h-[70vh] w-full overflow-hidden rounded-xl border border-gray-200">
      <div ref={containerRef} className="h-full w-full" aria-label="Mapa das Terras Indígenas do Brasil" />
      {status === "carregando" && (
